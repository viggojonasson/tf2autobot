...
export default class Pricelist extends EventEmitter {
    ...
    private handlePriceChange(data: any): void {
        ...
        if (match !== null && match.autoprice) {
            ...
            if (process.env.DISABLE_DISCORD_WEBHOOK_PRICE_UPDATE === 'false' && this.discordWebhookLinks.length !== 0) {
                ...
                this.sendWebHookPriceUpdateV1(match.sku, name, match, time);
                ...
                //     this.sendWebHookPriceUpdateV2(this.priceChanges);
    ...
    private sendWebHookPriceUpdateV1(sku: string, name: string, newPrice: Entry, time: string): void {
        ...
        const australiumImageURL = this.australiumImages();
        const paintCan = this.paintCanImages();
        const qualityColor = this.qualityColor();
    ...
    private australiumImages(): { [key: string]: string } {
        ...
        return australiumImageURL;
    }

    private paintCanImages(): { [key: string]: string } {
        const paintCan = {
    ...
    private qualityColor(): { [key: string]: string } {
        const qualityColor = {
    ...
    // For future reference - include old prices
